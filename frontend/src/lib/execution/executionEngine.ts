...
async function simulateNodeExecution(
  ...
  onLog(createLogEntry(nodeId, 'Processing...', 'info', 'start'));
  ...
  onLog(createLogEntry(nodeId, 'Done', 'success', 'end'));
}
...
export async function runPipeline(
  ...
  try {
    for (const nodeId of order) {
      ...
      try {
        await simulateNodeExecution(nodeId, (e) => callbacks.onLog?.(e));
        callbacks.onNodeState?.({ nodeId, status: 'completed' });
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        callbacks.onLog?.(
          createLogEntry(nodeId, message, 'error')
        );
